		...
		let penstockSumpDataFilterApiRequest = http.request({
			...
			res.on('end', () => {
				...
				// if(latestValue !== parseFloat(penstockSumpDataFilterApiResponse)) {
					data_db_connection_pool.query('INSERT INTO flood_monitoring_temp_filter(fms_id, real_value, replaced_value, time, parameter) VALUES (?,?,?,?,?)', [stationId, latestValue, JSON.stringify(penstockSumpDataFilterApiResponse), current_time, parameter]);
				// }
			});
		});
...
const main = (async () => {
	try {
		...
		setInterval(() => {
			...
			data_db_connection_pool.query('SELECT fms_id, fms_param_list, fms_lst_unprocessed_data, fms_lst_data, fms_params_last_updated_data, fms_params_last_update_time, fms_params_last_status FROM flood_monitoring_stations', (err, results) => {
				if(err) {
