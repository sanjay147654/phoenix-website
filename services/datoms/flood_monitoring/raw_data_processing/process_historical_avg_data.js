...
const timestamps = [1544812200, 1545676200];
const calculationType = ''; // '15_min', 'hourly', 'daily'
...
const process15MinAvgData = (fromTime = timestamps[0], finalTime = timestamps[1]) => {
	...
	fromTime = (fromTime - (fromTime % 900));
	...
	if (uptoTime <= finalTime) {
						...
						dataDB.query('SELECT tbl.fms_id, tbl.fms_rd_data FROM flood_monitoring_stations_raw_data AS tbl INNER JOIN (SELECT fms_id, MAX(fms_rd_time) max_time FROM flood_monitoring_stations_raw_data WHERE fms_rd_time < ? GROUP BY fms_id) AS max_tbl ON tbl.fms_id = max_tbl.fms_id AND tbl.fms_rd_time = max_tbl.max_time', [fromTime], (err, last_data_results) => {
							...
							allStations.map((station) => {
								...
								parameters.map((param) => {
									if (stationAvgData[param.key].count && param.type != 'pump_status') {
										if(param.type === 'rainfall') {
											let aggValue = ((stationAvgData[param.key].avg - 0) * ((param.type == 'rainfall') ? 25.4 : 1));
											stationAvgData[param.key].sum = aggValue.toFixed(3);
											stationAvgData[param.key].avg = aggValue.toFixed(3);
										} else {
											stationAvgData[param.key].avg = stationAvgData[param.key].sum / stationAvgData[param.key].count;
										}
									} else {
		...
		setTimeout(() => {
			process15MinAvgData(uptoTime);
		}, 10000);
	}
};

const processHourlyAvgData = (fromTime = timestamps[0], finalTime = timestamps[1]) => {
	console.log('processHourlyAvgData');
	/*let currentTime = moment.tz('Asia/Kolkata').unix(),
	...
	if (uptoTime <= finalTime) {
		...
		setTimeout(() => {
			processHourlyAvgData(uptoTime);
		}, 10000);
	}
};

const processDailyAvgData = (fromTime = timestamps[0], finalTime = timestamps[1]) => {
	console.log('processDailyAvgData');
	/*let currentTime = moment.tz('Asia/Kolkata').unix(),
		uptoTime = currentTime - (currentTime % 1800), //here 1800 is used instead of 3600, because all calculations are being made in IST (India/Kolkata)
...
const main = (async () => {
	try {
		...
		if(calculationType === '15_min') {
			process15MinAvgData();
		} else if(calculationType === 'hourly') {
			processHourlyAvgData();
		} else if(calculationType === 'daily') {
			processDailyAvgData();
		}
	} catch(err) {
		putToAppLog({
