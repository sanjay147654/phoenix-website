...
export default class SlideShow extends React.Component {
	/**
	 * This is the Constructor for SlideShow page used to set the default task while page loads.
	...
	componentWillUnmount() {
		...
		this.loop_interval = null;
		clearInterval(this.loop_interval_all);
		this.loop_interval_all = null;
	}
	...
	componentDidMount() {
		document.title = 'Slide Show - Flood Forecasting and Early Warning System for Kolkata City';
		// console.log('in mount');
		
		this.retriveData();
		this.setCookieToState();
		this.togglePlayPause(1);
		// this.loopForSlideShow();
	}

	dashboardSlideShow(data, update, slide_duration=null) {
		console.log('loop started dashboardSlideShow');
		let value_array = ['0-2-4', '0-2-7', '0-2-11'], label_array = ["Pump Station", "Street", "Rainfall"], sub_category_array = [4, 7, 11];
		let that = this, label_data = [], label_array_data, timer = slide_duration != null ? slide_duration : that.state.timer;
		if (that.state.sub_category_selected && that.state.sub_category_selected.length) {
			if (that.state.sub_category_selected[0] == 4) {
				label_array = ["Street", "Rainfall"];
				sub_category_array = [7, 11];
				timer = that.state.timer_duration * 2;
			} else if (that.state.sub_category_selected[0] == 7){
				label_array = ["Rainfall"];
				sub_category_array = [11];
				timer = that.state.timer_duration * 1;
			} else if (that.state.sub_category_selected[0] == 11){
				label_array = ["Pump Station", "Street", "Rainfall"];
				sub_category_array = [4, 7, 11];
				timer = that.state.timer_duration * 3;
			}
		}
		console.log('timer_1', timer);
		if (update) {
			label_data.push(label_array[0]);
			that.setState({
				label: label_data,
				timer: timer
			}, () => {
				...
				that.setSubCategoryForLoop(true, 2, sub_category_array[0]);
			});
			var count = 1;
			console.log('timer_3', timer);
			that.loop_interval = setInterval (() => {
				if (count < sub_category_array.length) {
					console.log('abc_', sub_category_array[count]);
					label_data.push(label_array[count]);
					that.setState({
						label: label_data,
						timer: timer
					}, () => {
						label_data = [];
						console.log('timer_3', timer);
						that.setSubCategoryForLoop(true, 2, sub_category_array[count]);
					});
				}
				count++;
			}, that.state.timer_duration);
		} else if (data) {
			var count = 0;
	...
	togglePlayPause(value) {
		let result;
		let data_time = (result = new RegExp('(?:^|; )' + encodeURIComponent('slide_duration') + '=([^;]*)').exec(document.cookie)) ? (result[1]) : 60000;
		this.setState({
			play_enabled: value == 1 ? true : false
		}, () => {
			console.log('here_now_', data_time);
			if (value == 1) {
				this.dashboardSlideShow(value, false);
				this.loop_interval_all = setInterval(() => {console.log('loop started loopForSlideShow'); this.dashboardSlideShow((value == true ? 1 : 0), true, data_time)}, data_time*3);
			} else {
				clearInterval(this.loop_interval);
				this.loop_interval = null;
				clearInterval(this.loop_interval_all);
	...
	gotoRespectivePage() {
		let query_string = '?cat=' + cat + '&subcat=[' + this.state.sub_category_selected + ']' + '&traffic=' + (this.state.traffic_map_view ? 1 : 0);
	...
	readCookie(key) {
		...
		// console.log('RegExp', new RegExp('(?:^|; )' + encodeURIComponent(key) + '=([^;]*)').exec(document.cookie));
	}

	setCookieToState() {
		let data_string_new = this.readCookie('slide_duration');
		console.log('data_string_new', data_string_new);
		this.setState({timer_duration: data_string_new ? data_string_new : '60000'});
	}

	setvalueToCookie(value) {
		let data_string = 'slide_duration='+ value;
		console.log('data_string', data_string);
		document.cookie = data_string;
		this.setCookieToState();
		clearInterval(this.loop_interval);
		this.loop_interval = null;
		clearInterval(this.loop_interval_all);
	...
	render () {
		...
		return (
			<div id="dashboard" className="slide-show">
				<Side active_link="slide-show" />
				<Head/>
				<Layout>
					{(() => {
						if (this.state.all_stations && this.state.all_stations.length) {
							return <Layout>
								<Content className="contains">
									...
									<div className="contain">
										<Row className="rows" className="top-0 data-icon-container">
											<Col xs={18} sm={9} md={9} lg={8} xl={5} className="data-head-title">{(this.state.label && this.state.label.length ? this.state.label[0] : '-') + ' data display'}</Col>
											<Col xs={4} sm={2} md={2} lg={1} xl={1} className="svg-icon-slide-show-container">
												{(() => {
													if (this.state.play_enabled) {
														return <Tooltip title="Click to Stop">
															<svg onClick={() => {this.togglePlayPause(0);}} className="svg-icon-slide-show" xmlns="http://www.w3.org/2000/svg" height="20" width="20" viewBox="0 0 508.52 508.52"><g fill="#010002"><path d="M254.26 0C113.845 0 0 113.845 0 254.26s113.845 254.26 254.26 254.26 254.26-113.845 254.26-254.26S394.675 0 254.26 0zm0 476.737c-122.68 0-222.477-99.829-222.477-222.477 0-122.68 99.797-222.477 222.477-222.477 122.649 0 222.477 99.797 222.477 222.477 0 122.648-99.829 222.477-222.477 222.477z"/><path d="M317.825 158.912h-127.13c-17.544 0-31.782 14.239-31.782 31.782v127.13c0 17.544 14.239 31.783 31.782 31.783h127.13c17.544 0 31.783-14.239 31.783-31.783v-127.13c-.001-17.543-14.239-31.782-31.783-31.782z"/></g></svg>
														</Tooltip>;
													} else {
														return <Tooltip title="Click to Play">
															<svg onClick={() => {this.togglePlayPause(1);}} className="svg-icon-slide-show" xmlns="http://www.w3.org/2000/svg" height="20" width="20" viewBox="0 0 30.051 30.051"><path d="M19.982 14.438l-6.24-4.536a.752.752 0 0 0-1.195.607v9.069a.75.75 0 0 0 1.195.606l6.24-4.532a.747.747 0 0 0 0-1.214z"/><path d="M15.026.002C6.726.002 0 6.728 0 15.028c0 8.297 6.726 15.021 15.026 15.021 8.298 0 15.025-6.725 15.025-15.021.001-8.3-6.727-15.026-15.025-15.026zm0 27.54c-6.912 0-12.516-5.601-12.516-12.514 0-6.91 5.604-12.518 12.516-12.518 6.911 0 12.514 5.607 12.514 12.518.001 6.913-5.603 12.514-12.514 12.514z"/></svg>
														</Tooltip>;
													}
												})()}
											</Col>
											<Col xs={24} sm={5} md={6} lg={9} xl={13} className="select-time-slide-show-container">
												<div className="select-time-slide-show-text-content">Slide Changing Interval :</div>
												<Tooltip title="Slider interval">
													<Select className="select-time-slide-show" placeholder="Slider interval" value={this.state.timer_duration} onChange={(e) => {this.setvalueToCookie(e)}}>
											...
											<Col xs={24} sm={7} md={6} lg={5} xl={4} className="link-to-go-dashboard">
												{(() => {
													if (this.state.sub_category_selected && this.state.sub_category_selected.length) {
														let query_string = '?cat=' + this.state.category_selected + '&subcat=[' + this.state.sub_category_selected + ']' + '&traffic=' + (this.state.traffic_map_view ? 1 : 0);
														return <Link to={'/' + query_string} >
															<span className="link-to-go-dashboard-inside">Go to Dashboard</span>
														</Link>;
													}
												})()}
											</Col>
										</Row>
										{/*<Col className="city-select" span={18}>
											...
											{(() => {
												if (this.state.view_mode === 'mobile' && this.state.show_index) {
													return <Row className="rows" id="label_container" className="label-container-mobile">
														{(() => {
															if (this.state.sub_category_selected && this.state.sub_category_selected.length && this.state.sub_category_selected.includes(7)) {
															...
															} else if (this.state.sub_category_selected && this.state.sub_category_selected.length && this.state.sub_category_selected.includes(4)) {
																return <div className="label-wrapper">
																	<div className="label-heading index">
																		For Pump Stations:
																	</div>
															...
															} else if (this.state.sub_category_selected && this.state.sub_category_selected.length && this.state.sub_category_selected.includes(11)) {
																return <div className="label-wrapper">
																	<div className="label-heading index">
										...
										{(() => {
											if (this.state.view_mode === 'desktop') {
												return <Row className="rows" id="label_container" className="label-container" type="flex" justify="start">
													{(() => {
														if (this.state.sub_category_selected && this.state.sub_category_selected.length && this.state.sub_category_selected.includes(7)) {
															return <div className="label-wrapper">
																{/*<div className="label-heading index">
																...
																<div className="label-content">
																	<div className="online-label status-content">Online</div>
																	<div className="offline-label status-content">Offline</div>
																</div>
															</div>;
														} else if (this.state.sub_category_selected && this.state.sub_category_selected.length && this.state.sub_category_selected.includes(4)) {
															return <div className="label-wrapper">
																...
																<div className="label-content">
																	<div className="pump-online-no-pump status-content">
																		No Pump is ON
																	</div>
																	<div className="pump-online-pump status-content">
																		Atleast 1 Pump is ON
																	</div>
																	<div className="pump-offline status-content">
																		Station Offline
																	</div>
																</div>
															</div>;
															...
															return <div className="label-wrapper">
																...
																<div className="label-content">
																	<div className="rain-online-1hr status-content">
																		Rainfall within last 1 hour
																	</div>
																	<div className="rain-online-24hrs status-content">
																		Rainfall within within last 24 hours
																	</div>
																	<div className="rain-online-no-rain status-content">
																		No rainfall within within last 24 hours
																	</div>
																	<div className="rain-offline status-content">
																		Station Offline
																	</div>
																</div>
															</div>;
